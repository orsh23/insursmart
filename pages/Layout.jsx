...
import React from 'react';
    ...
    function LayoutContent({ children }) { 
      ...
      const handleLogout = async () => {
        try {
          await User.logout();
          window.location.href = createPageUrl('index'); // Navigate to index page
        } catch (error) {
          console.error('Logout failed:', error);
        }
      };
      
      // Use the logical page name for the default/fallback path
      ...
      const menuItems = [
        ...
        { name: 'Address Management', icon: MapPin, path: createPageUrl('addressmanagement') }, // New Menu Item
      ...
      const currentMenuItem = menuItems.find(item => {
        ...
        // Handle root path specifically (e.g., if index page is '/')
        ...
        // For other paths, check if currentPath exactly matches or starts with item.path
        // This allows for nested routes like /settings/profile to match /settings
        ...
        <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex">
          ...
          >
            <div className="flex items-center justify-between h-16 px-4 border-b border-gray-200 dark:border-gray-700">
              {/* Ensure this link also uses the logical page name */}
            ...
            <nav className="mt-4 px-2 space-y-1 flex-grow flex flex-col">
              <div className="flex-grow">
                {menuItems.map((item) => {
                  // Normalize paths for comparison, including handling root
                  const normalizedItemPath = item.path.startsWith('/') ? item.path : `/${item.path}`;
                  const normalizedCurrentPath = currentPath.startsWith('/') ? currentPath : `/${currentPath}`;
