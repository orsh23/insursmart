...
export default function TasksPage() {
  ...
  const [currentView, setCurrentView] = useState(() => loadFromStorage('tasks_view_preference', 'card'));
  ...
  useEffect(() => {
    saveToStorage('tasks_view_preference', currentView);
  }, [currentView]);
  ...
  const taskPageActionsConfig = useMemo(() => {
    const currentUserRole = 'admin'; // This should ideally come from User.me() or context
    const permissions = getDefaultPermissionsForRole(currentUserRole);

    return buildTaskActionsConfig({
      t,
      toast,
      isSelectionModeActive,
      selectedItems,
      handleOpenTaskDialog,
      // The buildTaskActionsConfig expects specific handlers, so we pass them directly
      // For actions like Edit, if it's meant to edit a single item from context menu,
      // it needs to be handled within the component showing the item (e.g., TaskCard)
      // or the GlobalActionButton needs to be adapted.
      // For now, this config is for the main page actions.
      handleEditTask: (itemOrId) => { // More generic edit handler
        const taskToEdit = typeof itemOrId === 'string' ? allTasks.find(t => t.id === itemOrId) : itemOrId;
        if (taskToEdit) handleOpenTaskDialog(taskToEdit);
      },
      handleDeleteTasks: handleDeleteTasks, // For bulk delete and single delete from selection
      setIsImportDialogOpen,
      permissions,
      // Add a generic 'handlers' prop if buildTaskActionsConfig is designed to use it for its own internal actions
      handlers: { 
        onEditSingleItem: (itemId) => { // This is what buildTaskActionsConfig's EDIT action might use
            const taskToEdit = allTasks.find(t => t.id === itemId);
            if (taskToEdit) handleOpenTaskDialog(taskToEdit);
        }
      }
    });
  }, [t, toast, isSelectionModeActive, selectedItems, handleOpenTaskDialog, handleDeleteTasks, setIsImportDialogOpen, allTasks]);


  const handleExecuteGlobalAction = useCallback((actionType) => {
    switch(actionType) {
        ...
        case ActionTypes.EDIT: 
            if (selectedItems.length === 1) {
                ...
                if (taskToEdit) handleOpenTaskDialog(taskToEdit);
                 ...
                 toast({ title: t('common.notice'), description: t('tasks.bulkEditNotSupported', {defaultValue: "Bulk edit not yet supported. Please select one task."}), variant: 'info'});
            } else {
                 toast({ title: t('bulkActions.noItemsSelectedTitle'), description: t('bulkActions.selectItemsPrompt', { mode: t('common.edit')}), variant: 'info'});
            }
            break;
        case ActionTypes.DELETE: 
            ...
            break;
        default:
            console.warn(`Global action type ${actionType} not handled in TasksPage`);
            toast({title: t('common.notice'), description: t('errors.actionNotImplemented', {action: actionType}), variant: 'warning'});
    }
  }, [handleOpenTaskDialog, setIsImportDialogOpen, selectedItems, allTasks, handleDeleteTasks, t, toast]);
    ...
    <PageLayout>
      ...
      <div className="mb-4 flex flex-col sm:flex-row justify-between items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          <GlobalActionButton
              actionsConfig={taskPageActionsConfig} // Use the generated config
              isSelectionModeActive={isSelectionModeActive}
              selectedItemsCount={selectedItems.length}
              onAddNew={() => handleExecuteGlobalAction(ActionTypes.ADD)}
              onImportData={() => handleExecuteGlobalAction(ActionTypes.IMPORT)}
              // onExportData available if needed
              onEditSelected={() => handleExecuteGlobalAction(ActionTypes.EDIT)}
              onDeleteSelected={() => handleExecuteGlobalAction(ActionTypes.DELETE)}
              // Pass onExecuteAction for actions not directly handled by specific props
              onExecuteAction={handleExecuteGlobalAction} // This allows GlobalActionButton to call back for unhandled actions
              t={t} isRTL={isRTL}
            />
